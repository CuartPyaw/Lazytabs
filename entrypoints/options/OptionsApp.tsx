                        ...
                        <div className="grid gap-3">
                          ...
                          {draft.rules.map((rule, ruleIndex) => <section className="grid gap-3 border-t border-default pt-4" key={rule.id}>
                            {rule.conditions.map((condition, index) => <div className="grid grid-cols-1 items-center gap-2 sm:[grid-template-columns:12rem_9rem_minmax(0,1fr)_auto]" key={condition.id}>
                              ...
                              {(index === 0 && draft.rules.length > 1 || rule.conditions.length > 1) && <div className="flex gap-1 justify-self-end">{index === 0 && draft.rules.length > 1 && <Button isIconOnly aria-label={`删除第 ${ruleIndex + 1} 条规则`} size="sm" type="button" variant="tertiary" onPress={() => setRules(draft.rules.filter((_, itemIndex) => itemIndex !== ruleIndex))}><Trash2 size={16} strokeWidth={1.8} /></Button>}{rule.conditions.length > 1 && <button aria-label={`删除第 ${ruleIndex + 1} 条规则的第 ${index + 1} 个条件`} className="grid size-5 shrink-0 place-items-center rounded-full bg-danger text-danger-foreground" title="删除匹配条件" type="button" onClick={() => setConditions(ruleIndex, rule.conditions.filter((_, itemIndex) => itemIndex !== index))}><CircleMinus size={20} strokeWidth={2.1} /></button>}</div>}
                            </div>)}
                          </section>)}
