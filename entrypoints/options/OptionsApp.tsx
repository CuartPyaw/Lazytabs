                          ...
                          <div className="grid gap-2">
                            {draft.conditions.map((condition, index) => (
                              <div className="grid grid-cols-1 items-center gap-2 sm:[grid-template-columns:12rem_9rem_minmax(0,1fr)_auto]" key={condition.id}>
                                ...
                                {draft.conditions.length > 1 && <button aria-label={`删除第 ${index + 1} 条匹配规则`} className="grid size-5 shrink-0 place-items-center justify-self-end rounded-full bg-danger text-danger-foreground" title="删除匹配规则" type="button" onClick={() => setConditions(draft.conditions.filter((_, itemIndex) => itemIndex !== index))}><CircleMinus size={20} strokeWidth={2.1} /></button>}
                              </div>
                            ))}
